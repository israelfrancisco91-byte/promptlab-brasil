...
export default function PromptLabPage() {
  ...
  useEffect(() => {
    ...
    if (sharedData) {
      try {
        ...
        console.error("Erro ao ler o link:", error);
      }
    }
  }, []);
  ...
  // ================= FUNÇÃO DE LIMPEZA DE TEXTO (NOVA) =================
  // Essa função higieniza o texto copiado de qualquer site, removendo lixo invisível
  const cleanText = (text: string) => {
    if (!text) return "";
    return text
      .replace(/\r\n/g, '\n')
      .replace(/\r/g, '\n')
      // Converte todos os tipos de espaços especiais da web (\u00A0, etc) em espaços comuns
      .replace(/[\u00A0\u1680\u180E\u2000-\u200B\u202F\u205F\u3000\uFEFF]/g, ' ')
      // Converte tabulações (Tabs) em 4 espaços comuns para manter o alinhamento perfeito
      .replace(/\t/g, '    ');
  }
  // =====================================================================
  ...
  const generateShareLink = () => {
    try {
      const dataToShare = { header: repertoireHeader, songs: songs };
      ...
      navigator.clipboard.writeText(shareUrl).then(() => {
        ...
        alert("Erro ao copiar o link automaticamente.");
      });
    } catch (err) {
  ...
  const getPrompterLines = (content: string) => {
    const charLimit = 40; 
    // Aplicada a limpeza de texto aqui para o Teleprompter ficar perfeito também
  ...
  // --- MOTOR DE PDF INTOCÁVEL E PROTEGIDO ---
  const processPDF = async (action: 'download' | 'share') => {
    try {
      ...
      songs.forEach((song) => {
        ...
        // APLICAÇÃO DO FILTRO HIGIENIZADOR ANTES DE PROCESSAR AS LINHAS DO PDF:
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white font-sans p-4 relative">
            ...
            <section className="panel border-l-4 border-green-500 animate-in fade-in slide-in-from-bottom-4 duration-300">
              <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4 border-b border-slate-800 pb-6">
                ...
                <div className="flex w-full sm:w-auto gap-2 flex-wrap sm:flex-nowrap justify-end">
                  ...
                  <button onClick={generateShareLink} className="btn-icon !w-auto px-4 !bg-purple-600 hover:!bg-purple-500 text-xs font-bold uppercase" title="Gerar link de compartilhamento para banda">🔗 Compartilhar Link</button>
                  <button onClick={saveToLibrary} className="btn-icon !w-auto px-4 !bg-blue-600 hover:!bg-blue-500 text-xs font-bold uppercase" title="Salvar localmente na sua biblioteca">💾 Salvar</button>
                </div>
              </div>
                  ...
                  <div key={song.id} className="card relative group border-l-4 border-l-transparent focus-within:border-l-blue-500">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-3">
                      ...
                      <div className="flex flex-wrap gap-2 w-full md:w-auto md:pt-5 justify-end items-center">
                        <button onClick={() => openPrompter(song)} className="btn-play mr-2" title="Modo Palco">▶️ Prompter</button>
            ...
            <section className="panel border-l-4 border-blue-500">
              ...
              {savedRepertoires.length === 0 ? (
                <div className="text-center py-12 border-2 border-dashed border-slate-800 rounded-xl">
                  <h3 className="text-lg font-bold text-slate-300">Sua biblioteca está vazia</h3>
                </div>
              ) : (
                <div className="grid gap-4 md:grid-cols-2">
                  {savedRepertoires.map((rep) => (
                    <div key={rep.id} className="bg-[#1e293b] border border-slate-700 rounded-xl p-5">
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => loadFromLibrary(rep)} className="flex-1 bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs py-2 px-4 rounded-lg">Carregar</button>
          ...
          {activeTab === 'search' && (
            <section className="panel border-l-4 border-yellow-500">
              ...
              <div className="bg-[#1e293b] p-6 rounded-xl border border-slate-700 mb-8">
                <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Nome da música..." className="!mb-4 text-lg py-3 px-4" />
      ...
      {/* TEXTOS DE SEO E RODAPÉ (INALTERADOS) */}
      {!prompterSong && (
        <>
          <section className="max-w-3xl mx-auto mt-16 p-8 bg-[#0f172a] border border-slate-800 rounded-xl text-slate-400 text-sm leading-relaxed shadow-lg">
            ...
            <p className="mb-6">O <strong>PromptLab Brasil</strong> é a ferramenta definitiva para músicos, ministérios de louvor, corais e bandas que precisam organizar setlists de forma rápida e totalmente profissional...</p>
          </section>
      ...
      {prompterSong && (
        <div className="fixed inset-0 bg-[#020617] z-50 flex flex-col animate-in fade-in duration-300">
          <div className="bg-[#0f172a] border-b border-slate-800 p-4 flex flex-wrap gap-4 items-center justify-between shadow-2xl z-10">
            <button onClick={closePrompter} className="bg-slate-800 hover:bg-slate-700 text-white font-bold py-2 px-4 rounded-lg transition-colors">&larr; Voltar</button>
            <div className="flex items-center gap-2">
              <button onClick={() => setPrompterSpeed(Math.max(1, prompterSpeed - 1))} className="px-3 py-1 font-bold bg-slate-800 rounded">-</button>
              <span className="text-blue-400 font-bold">Vel: {prompterSpeed}</span>
              <button onClick={() => setPrompterSpeed(Math.min(10, prompterSpeed + 1))} className="px-3 py-1 font-bold bg-slate-800 rounded">+</button>
              <button onClick={() => setIsPrompterPlaying(!isPrompterPlaying)} className="ml-4 py-2 px-6 rounded font-bold bg-green-500/20 text-green-500 border border-green-500/50">{isPrompterPlaying ? '⏸ Pausar' : '▶️ Tocar'}</button>
            </div>
          </div>

          <div ref={prompterRef} className="flex-1 overflow-y-auto overflow-x-hidden p-6 sm:p-12 pb-[60vh]" style={{ fontSize: 'min(3.8vw, 2.5rem)', lineHeight: '1.6' }}>
            <div className="max-w-4xl mx-auto font-mono">
              <h1 className="text-5xl font-black mb-12 text-slate-500 border-b border-slate-800 pb-8">{prompterSong.title}</h1>
              {getPrompterLines(prompterSong.content).map((lineObj, idx) => {
                if (lineObj.type === 'empty') return <div key={idx} className="h-6"></div>;
                return <div key={idx} className={`prompter-line ${lineObj.type === 'chord' ? 'prompter-chord' : 'prompter-lyric'}`}>{lineObj.text}</div>;
              })}
            </div>
          </div>
        </div>
      ...
      {/* MODAL DE TEXTOS LEGAIS MANTIDO */}
    </div>
  )
}
