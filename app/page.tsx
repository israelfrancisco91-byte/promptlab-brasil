...
export default function PromptLabPage() {
  ...
  const [librarySearch, setLibrarySearch] = useState("") // NOVO: Estado da busca
  ...
  const filteredRepertoires = savedRepertoires.filter(rep => 
    rep.name.toLowerCase().includes(librarySearch.toLowerCase())
  );
  ...
  const handleToggleCase = (index: number) => {
    ...
    newSongs[index].content = lines.map(line => {
      if (isChordLine(line) || line.trim() === "") return line;
      if (isUpper) { const lower = line.toLowerCase(); return lower.charAt(0).toUpperCase() + lower.slice(1); }
      else { return line.toUpperCase(); }
    }).join('\n');
    setSongs(newSongs);
  };
  ...
  const handleToggleBold = (index: number) => {
    ...
    if (start === end) { alert("Selecione um trecho para colocar em negrito."); return; }
    const selected = text.substring(start, end);
    const newText = text.substring(0, start) + "**" + selected + "**" + text.substring(end);
    updateSong(index, 'content', newText);
    setTimeout(() => { textarea.focus(); textarea.setSelectionRange(start, end + 4); }, 10);
  };
  ...
  const processPDF = async (action: 'download' | 'share') => {
    try {
      ...
      const drawFixedElements = (pdfDoc: jsPDF) => {
        pdfDoc.setTextColor(150, 150, 150); pdfDoc.setFont("helvetica", "bold"); pdfDoc.setFontSize(10);
        ...
        while (j < lines.length) {
          ...
          if (isChordLine(chordLine) && lyricLine.trim() !== "" && !isChordLine(lyricLine)) {
            while (chordLine.length > 0 || lyricLine.length > 0) {
              ...
              const cChunk = chordLine.substring(0, breakIdx); const lChunk = lyricLine.substring(0, breakIdx);
              if (cChunk.trim() !== "") { doc.setFont("courier", "bold"); doc.setFontSize(10); doc.setTextColor(37, 99, 235); doc.text(cChunk.replace(/\*\*/g, ''), currentX, currentY); currentY += 4.5; }
              doc.setFontSize(10); doc.setTextColor(0, 0, 0);
              if (lChunk.includes('**')) { doc.setFont("courier", "bold"); doc.text(lChunk.replace(/\*\*/g, ''), currentX, currentY); } else { doc.setFont("courier", "normal"); doc.text(lChunk, currentX, currentY); }
              currentY += 6.5;
            ...
            while (remaining.length > 0) {
              ...
              const chunk = remaining.substring(0, breakIdx);
              doc.setFontSize(10); doc.setTextColor(0, 0, 0);
              if (isChordLine(line)) { doc.setFont("courier", "bold"); doc.setTextColor(37, 99, 235); doc.text(chunk.replace(/\*\*/g, ''), currentX, currentY); } 
              else { 
                if (chunk.includes('**')) { doc.setFont("courier", "bold"); doc.text(chunk.replace(/\*\*/g, ''), currentX, currentY); } 
                else { doc.setFont("courier", "normal"); doc.text(chunk, currentX, currentY); }
              }
              currentY += 5.5;
              remaining = remaining.substring(breakIdx + skipChars);
            }
            ...
            <section className="panel border-l-4 border-green-500 animate-in fade-in slide-in-from-bottom-4 duration-300">
              <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4 border-b border-slate-800 pb-6">
                ...
                <div className="flex w-full sm:w-auto gap-2">
                  <button onClick={clearCurrentSetlist} className="btn-icon !w-auto px-4 !bg-slate-800 hover:!bg-slate-700 text-xs font-bold uppercase">📄 Novo</button>
                  <button onClick={saveToLibrary} className="btn-icon !w-auto px-4 !bg-blue-600 hover:!bg-blue-500 text-xs font-bold uppercase shadow-[0_0_15px_rgba(37,99,235,0.4)]">💾 Salvar Repertório</button>
                </div>
              </div>
              
              <div className="mb-6">
                <label>Cabeçalho do PDF</label>
                <input value={repertoireHeader} onChange={(e) => setRepertoireHeader(e.target.value)} placeholder="Ex: Missa de Domingo" className="bg-[#0f172a]" />
              </div>
                ...
                {songs.map((song, index) => (
                  <div key={song.id} className="card relative group border-l-4 border-l-transparent focus-within:border-l-blue-500">
                    <div className="mb-3">
                      ...
                      <input value={song.title} onChange={(e) => updateSong(index, 'title', e.target.value)} placeholder="Ex: Te Louvarei" className="!mb-0 !bg-[#
